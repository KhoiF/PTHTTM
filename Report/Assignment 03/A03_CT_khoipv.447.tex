...
\vspace{0.4cm}
\noindent
Repository trên GitHub lưu toàn bộ mã nguồn notebook, dataset cấu trúc thư mục, hình ảnh báo cáo và file \LaTeX{} của Assignment 03.
Các notebook chính nằm trong thư mục \modelpath{src/Assigment 03}, còn file báo cáo này nằm trong \modelpath{Report/Assigment 03}.
\newpage

% =========================================================================
% CHUONG 1
% =========================================================================
\chapter{Cơ Sở Lý Thuyết Về Mạng Nơ-ron \& Học Biểu Diễn}

\section{Từ học máy truyền thống đến học biểu diễn}
...
\subsection{Vai trò của biểu diễn trong từng miền dữ liệu}
Trong Assignment 03, bốn bài toán có dạng dữ liệu khác nhau nên khái niệm ``học biểu diễn'' cũng có sắc thái khác nhau.
...
\subsection{Tổng quan bốn bộ dữ liệu thực nghiệm}
Bảng \ref{tab:dataset_overview} tóm tắt phạm vi dữ liệu được dùng trong báo cáo.
Các con số được lấy từ notebook và phần mô tả dataset hiện có trong \modelpath{src/Assigment 03}.
Mục tiêu của bảng không phải thay thế phần EDA chi tiết, mà là tạo một cái nhìn chung trước khi đi vào từng hệ thống.
...
\subsection{Nguyên tắc benchmark dùng xuyên suốt báo cáo}
Để kết quả giữa các mô hình có thể so sánh được, mỗi notebook đều giữ cùng một tinh thần thiết kế:
\begin{enumerate}
    ...
    \item Các đồ thị sinh ra trong notebook được lưu vào \modelpath{report/figures}, sau đó gom về \modelpath{Report/Assigment 03/images/generated} để tái sử dụng trong báo cáo.
\end{enumerate}
...
\section{Cấu trúc toán học của MLP}
...
\subsection{Ý nghĩa của biến đổi affine}
...
\section{Forward, Backward và Mini-batch Gradient Descent}
...
Mini-batch gom một số mẫu, ví dụ 128 hoặc 256, để tận dụng vectorization của NumPy nhưng vẫn cập nhật thường xuyên.
...
% CHUONG 2
% =========================================================================
\chapter{Thực Nghiệm Cơ Sở Trên Bộ Dữ Liệu Diabetes}

\section{Mục tiêu và dữ liệu}
Folder \modelpath{src/Assigment 03/diabetes} chứa hai notebook:
\begin{itemize}
...
\subsection{Vai trò của bộ diabetes nhỏ}
...
\section{Khảo sát dữ liệu và tiền xử lý}
...
\section{Huấn luyện MLP NumPy cải tiến}
...
% CHUONG 3
% =========================================================================
\chapter{Hệ Thống 1: Dự Đoán Tiểu Đường Quy Mô Lớn}

\section{Mục tiêu và cấu trúc dữ liệu}
Folder \modelpath{src/Assigment 03/diabetes\_large} mở rộng bài toán phân loại tiểu đường lên 100,000 mẫu. Dataset có các cột số như \texttt{age}, \texttt{bmi}, \texttt{HbA1c\_level}, \texttt{blood\_glucose\_level} và cột phân loại như \texttt{gender}, \texttt{smoking\_history}. Nhãn \texttt{diabetes} có tỷ lệ dương khoảng 8.5\%, do đó accuracy không đủ để đánh giá mô hình.
...
% CHUONG 4
% =========================================================================
\chapter{Hệ Thống 2: Định Giá Bất Động Sản}

\section{Mục tiêu và dữ liệu}
Folder \modelpath{src/Assigment 03/house\_price} xử lý bài toán hồi quy giá nhà từ dataset \modelpath{realtor-data.zip.csv}. Dataset gốc có 2,226,382 dòng, nhưng notebook dùng sample 200,000 dòng để bảo đảm thời gian chạy phù hợp trong môi trường học tập.
...
\section{EDA và feature engineering}
...
Nếu residual lớn tập trung ở phân khúc nhà cao cấp hoặc khu vực hiếm, hướng cải tiến không chỉ là đổi optimizer mà còn cần feature vị trí tốt hơn, ví dụ tọa độ địa lý, neighborhood hoặc chỉ số kinh tế địa phương.
...
% CHUONG 5
% =========================================================================
\chapter{Hệ Thống 3: Phân Loại Nhận Xét Khách Hàng}

\section{Mục tiêu và dữ liệu}
Folder \modelpath{src/Assigment 03/customer\_comments} sử dụng dataset \modelpath{Womens Clothing E-Commerce Reviews.csv}. Mục tiêu là dự đoán nhãn \texttt{Recommended IND} từ nội dung \texttt{Title + Review Text}. Sau khi bỏ review rỗng, còn 22,642 mẫu; tỷ lệ recommend chiếm đa số nên cần đánh giá bằng nhiều chỉ số.
...
\section{Làm sạch văn bản và TF-IDF}
...
% CHUONG 6
% =========================================================================
\chapter{Tổng Hợp Đối Chuẩn \& Kết Luận}

\section{Bảng tổng hợp các hệ thống}
Bảng \ref{tab:master_summary} tổng hợp mô hình tốt nhất của từng hệ thống theo metric chính. Điểm chung là không có một mô hình thắng tuyệt đối ở mọi miền dữ liệu; lựa chọn mô hình cần phụ thuộc vào dạng dữ liệu, chi phí sai lỗi, tài nguyên tính toán và yêu cầu giải thích.
...
\begin{tabular}{lllll}
\toprule
\textbf{Hệ thống} & \textbf{Dạng dữ liệu} & \textbf{Metric chính} & \textbf{Mô hình tốt nhất} & \textbf{Kết quả chính} \\
\midrule
Diabetes nhỏ & Bảng y sinh, 768 mẫu & F1 / Recall & Improved MLP & F1 0.8358, Recall 0.9655 \\
Diabetes large & Bảng y sinh, 100,000 mẫu & F1 / PR-AUC & Decision Tree & F1 0.8039, PR-AUC 0.8514 \\
House price & Bảng bất động sản, sample 200,000 & RMSLE / MAE & NumPy MLP & RMSLE 0.6578, MAE \$217,538 \\
Customer comments & Văn bản TF-IDF, 22,642 mẫu & F1 / ROC-AUC & Naive Bayes / Logistic Reg. & F1 0.9212, ROC-AUC 0.9348 \\
\bottomrule
\end{tabular}%
}
\end{table}
...
Trên house price, MLP đạt kết quả tốt nhất vì quan hệ giữa giá nhà, diện tích, vị trí, trạng thái giao dịch và lịch sử bán trước mang tính phi tuyến rõ. Tuy nhiên, sai số vẫn lớn do dữ liệu bất động sản có nhiều outlier, missing values và yếu tố vị trí chưa được mô hình hóa bằng tọa độ địa lý thực.
...
Các notebook đều cố định \texttt{RANDOM\_STATE = 42}, dùng split rõ ràng và fit các bước tiền xử lý chỉ trên training set. Những notebook sinh đồ thị đều lưu ảnh vào \modelpath{report/figures} tương ứng, sau đó ảnh được gom vào \modelpath{Report/Assigment 03/images/generated} để phục vụ báo cáo.
...
\begin{itemize}
    \item \modelpath{src/Assigment 03/diabetes/notebook/diabetes\_improved.ipynb}
    \item \modelpath{src/Assigment 03/diabetes\_large/notebook/diabetes\_large\_benchmark.ipynb}
    \item \modelpath{src/Assigment 03/house\_price/notebook/house\_price\_benchmark.ipynb}
    \item \modelpath{src/Assigment 03/customer\_comments/notebook/customer\_comments\_benchmark.ipynb}
\end{itemize}

\section{Kết luận}
Báo cáo đã hiện thực hóa và đối chuẩn MLP thuần NumPy trên bốn miền dữ liệu: diabetes nhỏ, diabetes large, house price và customer comments. Các kết quả cho thấy:
\begin{enumerate}
...
Hướng mở rộng tiếp theo gồm tối ưu ngưỡng theo chi phí nghiệp vụ, thử thêm gradient boosting cho dữ liệu bảng, dùng embedding/Transformer cho văn bản và bổ sung dữ liệu vị trí địa lý cho bài toán giá nhà.
